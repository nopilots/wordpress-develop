...
HTML
	...
	/**
	 * Ensures that CDATA sections inside MathML foreign content are detected.
	 *
	 * @ticket 61576
	 *
	 * @covers WP_HTML_Tag_Processor::next_token
	 */
	public function test_cdata_in_mathml_foreign_content() {
		$processor = new WP_HTML_Tag_Processor( '<math><![CDATA[x < y && y > z]]></math>' );
		$processor->next_token();

		// Artificially change namespace; this should be done in the HTML Processor.
		$processor->change_parsing_namespace( 'math' );
		$processor->next_token();
		...
		$this->assertSame(
			'x < y && y > z',
			$processor->get_modifiable_text(),
			'Found incorrect modifiable text in MathML CDATA section.'
		);
	}

	/**
	 * Ensures that an incomplete CDATA section inside foreign content pauses the parser.
	 *
	 * When a CDATA section is opened inside foreign content but the closing `]]>` is
	 * never found, the parser should report incomplete input rather than fall through
	 * to the bogus-comment path.
	 *
	 * @ticket 61576
	 *
	 * @covers WP_HTML_Tag_Processor::next_token
	 */
	public function test_incomplete_cdata_in_foreign_content() {
		$processor = new WP_HTML_Tag_Processor( '<svg><![CDATA[no closing sequence' );
		$processor->next_token();
		...
		$this->assertFalse(
			$processor->next_token(),
			'Should have failed to find the next token in an incomplete CDATA section.'
		);

		$this->assertTrue(
			$processor->paused_at_incomplete_token(),
			'Should have paused at incomplete CDATA token.'
		);
	}

	/**
	 * Ensures that a non-CDATA bogus comment inside foreign content is still treated
	 * as an invalid-HTML comment (not as a CDATA section).
	 *
	 * @ticket 61576
	 *
	 * @covers WP_HTML_Tag_Processor::next_token
	 */
	public function test_non_cdata_bogus_comment_in_foreign_content() {
		$processor = new WP_HTML_Tag_Processor( '<svg><![NOT_CDATA[something]]></svg>' );
		$processor->next_token();
		...
		$this->assertSame(
			'#comment',
			$processor->get_token_name(),
			"Should have found a comment token but found {$processor->get_token_name()} instead."
		);

		$this->assertSame(
			WP_HTML_Processor::COMMENT_AS_INVALID_HTML,
			$processor->get_comment_type(),
			'Should have detected an invalid HTML comment for non-CDATA bogus comment.'
		);
	}
